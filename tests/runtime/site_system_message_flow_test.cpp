...
template <typename System, typename Message>
constexpr bool system_subscribes_to_message()
{
    return gs1::type_list_contains_v<Message, typename System::subscribed_messages>;
}
}  // namespace
...
int main()
{
    ...
    const DeploymentSiteSelectionChangedMessage selection_changed {3U};
    const bool loadout_subscribes =
        system_subscribes_to_message<LoadoutPlannerSystem, DeploymentSiteSelectionChangedMessage>();
    assert(loadout_subscribes);
    const auto selection_changed_status =
        invoke_system_message<LoadoutPlannerSystem, DeploymentSiteSelectionChangedMessage>(
            selection_changed,
            app_state,
            campaign_state,
            no_site_run,
            runtime_queue,
            runtime_messages);
    assert(selection_changed_status == GS1_STATUS_OK);
    ...
    const DeploymentSiteSelectionChangedMessage selection_cleared {0U};
    const auto selection_cleared_status =
        invoke_system_message<LoadoutPlannerSystem, DeploymentSiteSelectionChangedMessage>(
            selection_cleared,
            app_state,
            campaign_state,
            ...
            runtime_queue,
            runtime_messages);
    assert(selection_cleared_status == GS1_STATUS_OK);
    assert(!campaign_state->loadout_planner_state.selected_target_site_id.has_value());
    ...
    const StartSiteAttemptMessage start_site_attempt {1U};
    const auto start_site_attempt_status =
        invoke_system_message<CampaignFlowSystem, StartSiteAttemptMessage>(
            start_site_attempt,
            app_state,
            active_campaign,
            active_site_run,
            site_start_queue,
            runtime_messages);
    assert(start_site_attempt_status == GS1_STATUS_OK);
