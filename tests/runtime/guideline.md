...
- `game_runtime_projection_test.cpp`: Verifies projected runtime/UI state output, including regional-map support projection, faction-tab tech-tree UI projection, single-crate site bootstrap and loadout projection, tile-upsert delta coalescing for quantized visible density changes, weather-event timeline projection, inventory presentation, authoritative phone panel state emission with task counts, phone listing add/remove projection behavior, and runtime delivery pacing under the dedicated time-system architecture.
