...
- `game_runtime_projection_test.cpp`: Verifies projected runtime/UI state output, including regional-map support projection, faction-tab tech-tree UI projection from both the regional map and active site sessions, single-crate site bootstrap and loadout projection, tile-upsert delta coalescing for quantized visible density, local-wind, and ecology-driven terrain-appearance changes, weather-event timeline projection, inventory presentation, authoritative phone home/app-panel state emission with task counts, phone listing add/remove projection behavior, phone section-change updates, and runtime delivery pacing under the dedicated time-system architecture.
