...
- `action_ecology_placement_system_tests.cpp`: Cross-system regression tests for actions, ecology, placement, and multi-tile plant wind-exposure behavior.
- `asset_regression_runners.cpp`: Registers and implements runtime runners for asset-authored system tests.
- `campaign_progress_system_tests.cpp`: System tests for campaign progression behavior, including selected-site support assembly, faction-reputation rewards, occupied-reputation tech claims, tier unlock rules, amplification lockout, and regional-map tech-tree tab/open flow.
...
- `environment_worker_device_system_tests.cpp`: System tests across environment, worker, and device interactions, including directional wind-shadow coverage for plants and windbreak devices.
