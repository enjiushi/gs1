...
- `action_ecology_placement_system_tests.cpp`: Cross-system regression tests for actions, ecology, placement, harvest-output insertion plus density setbacks, persistent item-based plant-mode rearming, checkerboard `2x2` plant occupancy, previous-frame work-efficiency-driven action timing/cost, multi-tile plant wind-exposure behavior, summed owner-specific weather-contribution-driven terrain updates, and highway-target sand-cover handling.
...
- `environment_worker_device_system_tests.cpp`: System tests across environment, worker, and device interactions, including prototype Site 1 bootstrap layout for the harvestable starter plant patches near camp, authored hostile site-one background weather, owner-specific plant/device weather contribution accumulation, directional wind-shadow coverage for plants, plant-tile projection dirties on local wind changes, derived worker energy-cap/work-efficiency behavior, calibrated worker passive weather-decay timing and heat-wind-dust ratio coverage, site-time-system-aligned weather timeline interpolation, windbreak devices, and one-sided highway-protection weather waves.
- `inventory_economy_task_system_tests.cpp`: System tests spanning inventory, economy, site-start delivery-crate loadout seeding, immediate delivery-crate insertion plus overflow queuing, the expanded balanced procedural task-board pool, zero-reward task completion, resolved per-instance buy/transfer/build targets, owner-snapshot-driven task-board startup state, worker-plus-plant duration-task progress, duration/build-task progress, and phone sell/cart regressions.
