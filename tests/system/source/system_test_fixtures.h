...
inline constexpr bool system_declares_message_subscription_v =
    requires { typename System::subscribed_messages; } &&
    type_list_contains_v<Message, typename System::subscribed_messages>;

template <typename System, typename Message>
inline Gs1Status invoke_typed_system_message(
    System& system,
    RuntimeInvocation& invocation,
    const Message& message)
{
    if constexpr (requires(System& candidate, RuntimeInvocation& runtime_invocation, const Message& payload) {
                      { candidate.handle(runtime_invocation, payload) } -> std::convertible_to<Gs1Status>;
                  })
    {
        return system.handle(invocation, message);
    }
    else
    {
        (void)system;
...
struct SplitRuntimeFixture final
{
        ...
        GameMessageQueue& message_queue)
    {
        ...
        if (state.campaign_core.has_value())
        {
            ...
            apply_campaign_state_from_state_sets(*campaign, state, state_manager);
        }
        else
        {
            campaign.reset();
        ...
        if (state.site_run_meta.has_value())
        {
            ...
            apply_site_run_state_from_state_sets(*active_site_run, state, state_manager, site_world);
        }
        else
        {
    ...
    SiteMoveDirectionInput move_direction = {})
{
    ...
    fixture.sync(invocation, app_state, campaign, active_site_run, message_queue);
    return status;
}
...
inline Gs1Status invoke_system_message(
    ...
    double fixed_step_seconds = k_default_fixed_step_seconds)
{
    Gs1AppState app_state = context.campaign.app_state;
    std::optional<CampaignState> campaign {context.campaign};
    std::optional<SiteRunState> active_site_run {};
    GameMessageQueue message_queue {};
    const auto status = invoke_system_message<System>(
        message,
        app_state,
        campaign,
        active_site_run,
...
inline Gs1Status invoke_system_message(
    ...
    double fixed_step_seconds = k_default_fixed_step_seconds)
{
    return invoke_system_message<SystemTag>(
        message,
        context.app_state,
        context.campaign,
    ...
    const GameMessage& message)
{
    ...
    const auto status = invoke_system_message<SystemTag>(
        message,
        app_state,
        campaign,
        active_site_run,
