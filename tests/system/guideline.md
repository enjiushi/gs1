...
- `source/`: Source-authored system tests compiled directly into the standalone host, including harvest-action, harvested-output, and excavation-action regressions plus linear `32`-tier faction-tech progression coverage, total-reputation unlock gating across multiple content kinds, Village recipe gating, shovel carry bonuses, pack-slot growth, and timed-buff-slot coverage.
