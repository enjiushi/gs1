...
- `source/`: Source-authored system tests compiled directly into the standalone host, including harvest-action, harvested-output, and excavation-action regressions with reduced dedicated-output CP budgets plus independent seed/dry-grass secondary-roll coverage, linear `32`-tier faction-tech progression coverage, shared total-reputation unlock gating across the `100`-step basic food/drink ladder plus the non-starter plant roster and the starter Field Kitchen/Workbench/Storage Crate/Hammer/Wood/Iron/Medicine utility path, later Chemistry Station/Herbal Poultice/Wind Fence progression, Village stronger-recipe gating, shovel carry bonuses, pack-slot growth, timed-buff-slot coverage, and the split-state test bridge helpers used by the runtime/system fixture harnesses, including in-place apply/write paths for round-trips.
