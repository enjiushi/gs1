...
- `smoke_engine_host.h`: Smoke host abstraction over the gameplay DLL runtime, including cached campaign-resource projection for visual smoke clients.
- `smoke_engine_host.cpp`: Smoke host implementation, including authoritative site phone home/app-panel state with completed/claimed task counts, site-tile local-wind projection intake for viewer-reactive plants, weather-event timeline projection application, campaign cash/total-reputation projection intake, and logging names for regional-map tech-tree UI actions/setups.
