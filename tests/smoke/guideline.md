...
- `smoke_engine_host.h`: Smoke host abstraction over the gameplay DLL runtime, including projected site-tile ecology channels used by the visual terrain viewer.
- `smoke_engine_host.cpp`: Smoke host implementation, including authoritative site phone home/app-panel state with completed/claimed task counts, site-tile local-wind plus ecology-meter projection intake for viewer-reactive plants and terrain shading, weather-event timeline projection application, and logging names for regional-map tech-tree UI actions/setups.
...
- `smoke_live_state_json.cpp`: JSON serialization of live smoke/runtime state, including phone home/app-panel section/count snapshot fields, claimed-task list-kind names, site-tile local-wind plus ecology-meter fields for plant animation and terrain shading, weather-event timeline fields, and regional-map tech-tree tab/action/setup names.
