...
- `shared_framework_process.md`: Standard workflow for extracting the reusable runtime/host/Godot framework into a shared upstream repo, vendoring snapshots into game repos such as `gs1` and `gs2`, and keeping `godot-cpp` as a game-owned dependency instead of making the shared framework repo own duplicated Godot headers.
