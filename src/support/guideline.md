...
- `packed_id_index.h`: Shared packed-storage side-index helper that maps stable ids or composite lookup keys to packed vector indices without making the packed data containers themselves own sparse gaps or unstable external ids.
- `site_objective_types.h`: Shared enums for site objective modes and target edges used by content, site setup, and objective evaluation, now including the cash-target survival mode.
