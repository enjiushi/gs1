...
- `runtime_split_state_compat.h`: Shared split-state-to-aggregate compatibility mapping helpers kept for test seeding/assertion helpers plus runtime compatibility assembly/write-back for callers that still operate on aggregate `CampaignState`, `SiteRunState`, or aggregate campaign/site sub-slices while authoritative runtime state remains split into one-container-or-trivial pointer-free state sets.
