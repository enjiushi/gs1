...
- `game_runtime.cpp`: Runtime loop implementation, integration wiring, projected engine/HUD message emission, tile-upsert coalescing against the last emitted visible tile snapshot including fine plant-density buckets for withering hints plus quantized local-wind and protection-channel projection for plant-reactive viewers and protection heatmaps, temporary Site 1 starter-tile projection probe logs for sent-versus-suppressed tile upserts now marked as debug-level so activity-only smoke logging suppresses them, authoritative phone home/app-panel state plus tracked phone-open visibility and phone listing/task-count projection emission, tracked worker-pack and device-storage inventory-view projection with stable already-open device snapshots while the worker-pack panel toggles, site-panel exclusivity coordination between inventory, phone, tech-tree, and the Alt-triggered protection selector/overlay state, claimed-task list-kind projection, site-result `OK` return action projection, regional-map support previews plus authored tile-position projection, prototype tech-tree UI projection available from both the regional map and active site sessions with one-by-one total-reputation plant unlocks, faction base-tech rows plus exclusive enhancement choices, optional tech-granted content readouts, faction-tab reputation summaries, money purchases, reputation-scaled effect-parameter readouts, and top-down refunds, while treating batched ecology task-tracking summaries as internal gameplay traffic rather than direct projection triggers.
