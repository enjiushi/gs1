...
- `runtime_state_access.h`: Tagged runtime-state access declarations plus `RuntimeInvocation` plumbing for authoritative app/campaign/site state-set access, fixed-step timing, message queues, and move-direction snapshots, now with explicit tagged owner mapping plus const-only non-owner reads so `GameStateAccess::write()` stays owner-only.
