...
- `game_runtime.cpp`: Runtime loop implementation, integration wiring, projected engine/HUD message emission, tile-upsert coalescing against the last emitted visible tile snapshot including quantized local-wind projection for plant-reactive viewers, authoritative phone panel state plus phone listing/task-count projection emission, claimed-task list-kind projection, site-result `OK` return action projection, regional-map support previews, and regional-map faction tech-tree UI projection.
