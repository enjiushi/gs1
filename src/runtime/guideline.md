...
- `runtime_split_state_compat.h`: Shared split-state-to-aggregate helper mappings kept for test seeding/assertion helpers and other narrow non-production aggregate callers while authoritative runtime state remains split into one-container-or-trivial pointer-free state sets.
