...
- `technology_system.cpp`: Technology system implementation for the shared total-reputation unlock ladder across plants/items/recipes/device-build recipes, including the baseline-on-start Camp Stove, Storage Crate, and Ephedra Stew recipe exceptions plus the rescaled unlock ladder that now starts Shovel at `40` total reputation and then advances in `200`-rep steps, alongside the linear `32`-tier-per-faction tech ladder gated by matching faction-reputation requirements, top-down money refunds by purchased tier, internal-cash-point campaign spending/refunds behind player-facing cash labels, and reputation-scaled tech-effect parameter helpers.
