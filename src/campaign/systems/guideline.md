...
- `technology_system.cpp`: Technology system implementation for the shared total-reputation baseline unlock ladder across the non-starter plant roster plus simple shared food, including the baseline-on-start Field Kitchen, Workbench, Storage Crate, Hammer, Medicine, Ephedra Stew, and Shovel recipe exceptions, alongside the linear `32`-tier-per-faction tech ladder that now auto-unlocks directly from matching faction-reputation requirements, treats faction-granted recipes/items/structures as authoritative unlock sources, and exposes reputation-scaled tech-effect parameter helpers for the revised Village/Bureau/University split.
