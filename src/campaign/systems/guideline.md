...
- `campaign_time_system.h`: Campaign time system interface for fixed-step-derived campaign clock advancement through the shared `RuntimeInvocation` system interface, without separately registering `CampaignCore` ownership now that campaign flow remains the single registered owner for that state set.
- `campaign_time_system.cpp`: Campaign time system implementation that advances campaign clock/day progression inside split `CampaignCore` state while campaign flow remains the single registered resolver for that state set during runtime startup.
