...
- `campaign_flow_system.cpp`: Campaign flow system implementation for campaign state transitions, authoritative runtime app-state handoff, tech-tree open/close and faction-tab flow from both the regional map and active site sessions, direct host gameplay action handling for start/select/deploy/return intents without first bouncing through a centralized router or synthetic gameplay request messages, site-completion reward emission through `RuntimeInvocation`, and adjacent-site availability promotion without duplicating already-visible prototype site markers in the revealed-site cache.
