...
- `technology_system.cpp`: Technology system implementation for the shared total-reputation unlock ladder across plants/items/recipes/device-build recipes, including the baseline-on-start Camp Stove, Storage Crate, Ephedra Stew, and Shovel recipe exceptions plus the later `200`-step unlock ladder for the remaining authored reputation-gated content, alongside the linear `32`-tier-per-faction tech ladder that now auto-unlocks directly from matching faction-reputation requirements and exposes reputation-scaled tech-effect parameter helpers.
