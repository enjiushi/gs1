...
- `campaign_presentation_system.h`: Campaign presentation runtime-system interface for regional-map/menu/tech-tree/resource/result/unlock cue projection follow-up through the normal subscriber model.
- `campaign_presentation_system.cpp`: Campaign presentation runtime-system implementation that owns runtime UI/presentation state for campaign-regional projection follow-up, emits regional-map UI and snapshot messages, pushes campaign-resource and unlock cues after gameplay messages settle, and removes that regional presentation work from the app-layer coordinator path.
