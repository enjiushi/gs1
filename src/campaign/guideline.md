...
## Contents
- `campaign_state.h`: Aggregate compatibility campaign state retained for factories, tests, and transitional read paths, while authoritative runtime-owned campaign storage is now split into separate state sets for core, regional-map, faction-progress, technology, loadout-planner, and site metadata ownership.
