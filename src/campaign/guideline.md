...
## Contents
- `campaign_state.h`: Aggregate compatibility campaign state retained for factories, tests, and transitional read paths, while authoritative runtime-owned campaign storage is now split into separate state sets for core, regional-map meta plus per-list slices, faction-progress, technology, loadout-planner meta plus per-list slices, and flattened site metadata slices. State-set payloads in the split path are kept pointer-free.
