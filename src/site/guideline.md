...
- `systems/`: Site system declarations and implementations, including harvest action execution, ecology harvest resolution, multi-tile planting completion now aligned back to the footprint anchor before ecology applies occupancy updates, worker-pack harvest insertion, rep-aware shop/crafting/task filtering, and the Village-specific shovel, excavation-depth, buff, and custom-food runtime hooks.
