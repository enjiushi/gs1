...
- `site_run_state.h`: Aggregate active site-run state that owns the site slice, including pending projection dirtiness, cached last-reported tile-density values for cumulative ecology message gating, retained plant/device weather-contribution dirty state, cached last-emitted tile projection state for delta coalescing across visible tile data, local-wind-driven plant visuals, and ecology-owned counters such as living-plant stability status.
