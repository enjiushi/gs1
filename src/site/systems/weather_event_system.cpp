    ...
    const ConstTaskBoardStateRef& board) noexcept
{
    for (const auto& task : board.visible_tasks)
    {
        ...
        if (find_site_onboarding_task_seed_def(site_id, task.task_template_id) != nullptr)
        {
            return true;
        }
    }
...
const char* WeatherEventSystem::name() const noexcept
{
...
std::optional<std::uint32_t> WeatherEventSystem::fixed_step_order() const noexcept
{
    return 4U;
}
