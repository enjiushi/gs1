...
- `action_execution_system.cpp`: Executes queued site actions using the canonical real-seconds-to-action-minutes conversion, derives action duration plus deferred worker action costs from the previous-frame work-efficiency snapshot, scales authored worker meter costs from the worker tile's current local weather when the action starts including morale, applies those stored costs when the action completes, reads recipe-authored craft duration and worker meter costs, emits follow-up gameplay messages, supports harvest actions for density-ready plants, resolves rough/careful/thorough excavation state with deterministic depth-specific tier-biased stone loot, and re-arms plant placement mode after successful item-based planting when matching seeds remain in the worker pack.
...
- `modifier_system.cpp`: Applies and expires site modifiers/effects using content-authored nearby-aura and run-modifier preset tables, including task-reward run-modifier awards plus imported campaign assistant effects and faction-tech global-modifier effects scaled by current faction reputation, and resolves terrain factor weights/biases from the active modifier totals with content-authored clamp and camp-comfort tuning.
- `phone_panel_system.h`: Phone panel system interface.
- `phone_panel_system.cpp`: Owns phone home/app-panel section state plus tracked phone open-close visibility and authoritative projected phone listings/task counts for the adapter, including completed and claimed history counts plus derived sell-list pricing from internal cash-point item values, while intentionally filtering frozen contractor offers out of the current player-facing phone projection.
