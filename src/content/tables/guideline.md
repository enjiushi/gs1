...
- `items.toml`: Item authoring content for runtime item definitions and item-domain metadata, including consumable meter deltas, internal cash-point valuation used by task/reward scoring, the full prototype seed-item roster, harvested-good items for the China-desert ten-plant set, and excavation-only stone merchandise rows.
- `excavation_tables.toml`: Excavation depth tuning and tier-scoped `100%` loot pools for the prototype desert stone finds.
...
- `gameplay_tuning.toml`: Shared numeric gameplay-tuning table for worker-condition, ecology, modifier, device-support, and camp-durability factors, including worker background energy-recovery tuning, the current higher base plant-pressure floor with lighter dust weighting, the shared `0.01` weather-vs-resistance growth-pressure comparison that should remain data-authored instead of hardcoded, and the ecology full-range real-minutes tuning for the linear density-change curve, currently tuned to a `5` real-minute full-range baseline.
