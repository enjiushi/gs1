...
- `task_templates.toml`: Authored task template content using single target/required-count/threshold values, where `0` or `None` means the runtime must resolve that field procedurally, plus optional selector hints for runtime-picked item/plant/recipe/structure/action values.
...
- `site_actions.toml`: Site action tuning content for shared action duration and worker energy/hydration/nourishment costs, authored local-weather-to-meter cost coefficients, placement occupancy, and reservation/approach/movement flags, with craft duration and worker meter costs authored per recipe and the shared harvest action profile still loaded here.
