...
- `technology_tiers.toml`: Faction tech-tree tier content for tier ordering and display names, currently authored through tier 8 with tiers 4-8 left empty of tech nodes for future expansion.
...
- `technology_nodes.toml`: Technology node content for one faction-owned tech per tier row slot, authored faction reputation requirements plus internal cash-point valuation that converts to player-facing cash, reputation-scaled effect parameters, entry kinds, and placeholder labels/descriptions.
