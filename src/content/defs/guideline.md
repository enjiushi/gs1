...
- `technology_defs.h`: Prototype progression definition records and runtime accessors, including the current 8 authored tier-row tech slots, per-node faction reputation requirements plus internal cash-point valuation that converts to player-facing cash, authored reputation-scaled effect parameters, per-plant total-reputation unlock thresholds, starter plants available from campaign start, and placeholder faction tech nodes.
