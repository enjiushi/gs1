...
- `task_defs.h`: Task board/authored task records plus runtime accessors, including single-value target/required-count/threshold fields that use `0` or `None` as procedural sentinels, expanded progress kinds such as living-plant non-withering duration checks, optional action/item/plant/recipe/structure selector hints, and site-keyed onboarding task seed overrides with optional fixed reward candidates.
