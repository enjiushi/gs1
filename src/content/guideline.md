...
- `content_validator.cpp`: Validation checks for loaded authored content, campaign/site graph/setup references, site default-weather and starter-plant bootstrap values, shared gameplay-tuning ranges, seeded phone listings, single-value task-template references, site-keyed onboarding task seed references, site action coverage plus non-negative worker meter costs and weather-to-action-cost coefficients, harvest-item coherence, modifier preset coverage, neutral-tech and faction-enhancement progression references, and cross-file references.
