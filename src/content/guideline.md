...
## Contents
- `content_database.h`: Aggregate content database types that hold loaded definitions, including the shared gameplay-tuning slice consumed by multiple runtime systems.
...
- `content_validator.cpp`: Validation checks for loaded authored content, campaign/site graph/setup references, site default-weather and starter-plant bootstrap values, shared gameplay-tuning ranges, seeded phone listings, expanded task-template item/plant/structure references, site action coverage plus non-negative weather-to-action-cost coefficients, harvest-item coherence, modifier preset coverage, neutral-tech and faction-enhancement progression references, and cross-file references.
