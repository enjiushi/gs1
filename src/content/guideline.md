...
- `content_loader.cpp`: Config-table parsing and startup loading for the externalized prototype content slices, including the table-backed campaign/site graph, campaign setup, site phone-listing seeds, site action tuning with per-action worker nourishment cost, modifier presets, technology progression authoring, and multi-channel structure support values used by resolved tile computation.
