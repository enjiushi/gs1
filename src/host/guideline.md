...
- `runtime_session.h`: Shared gameplay-runtime session owner that loads the gameplay DLL, creates the runtime from a host-supplied shared project-config root plus fixed-step timing, advances the two-phase update loop, and exposes host-event submission for engine adapters that need to drive gameplay UI/runtime flow directly.
- `runtime_session.cpp`: Implementation of the shared gameplay-runtime session owner, including shared host-event submission into the gameplay DLL runtime and startup wiring for the shared project-config root path passed across the runtime ABI.
