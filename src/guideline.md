...
- `host/`: Engine-agnostic runtime-DLL host bridge code shared by process/system test hosts and future engine adapters, including the shared DLL loader, runtime-session wrapper around split phase execution plus typed host-command submission, adapter-config helpers, adapter-metadata loading, and wrappers for the read-only gameplay-state view and tile-query ABI.
- `messages/`: Internal gameplay message contracts and dispatch helpers.
- `runtime/`: Core runtime loop, clocks, execution services, split gameplay state-set storage, `StateManager` ownership/query wiring, and the exported read-only state-view/tile-query helpers used by hosts and adapters, with the manager now holding the aggregate `GameState`, the current architecture direction moving internal gameplay messages toward inline nested dispatch rather than deferred gameplay-message drains, and the new refactor scaffolding starting to define compile-time type lists, canonical system-pack ordering, and typed-gameplay-message bridge helpers for the in-progress internal dispatch migration.
- `site/`: Active site-run state, ECS world helpers, and site-owned systems, including split-backed site-state access helpers, harvest action execution plus worker-pack harvest output routing, campaign-aware recipe/item unlock filtering, Village-specific shovel, excavation, buff, and custom-food runtime hooks, and now dedicated runtime-system site presentation follow-up for action/cue/protection UI reactions instead of app-layer post-message coordination, with system entrypoints constructing their own local runtime contexts directly instead of routing through file-local context wrapper helpers when no extra abstraction is needed. Later-phase task board/modifier/economy ECS promotion is tracked separately in `ECS_STATE_TO_ECS_PLAN.md`.
