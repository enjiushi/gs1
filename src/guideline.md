...
- `runtime/`: Core runtime loop, queues, clocks, execution services, split gameplay state-set storage, `StateManager` ownership/query wiring, and the exported read-only state-view/tile-query helpers used by hosts and adapters, with the manager now holding the aggregate `GameState`.
