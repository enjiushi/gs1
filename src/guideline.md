...
- `host/`: Engine-agnostic runtime-DLL host bridge code shared by smoke hosts and future engine adapters, now including the shared projection cache that carries app/menu, regional-map, dense row-major site-tile, inventory/task/phone/craft/placement/protection, and recent-cue state alongside the drained engine-message stream used by the Godot adapter and smoke hosts, plus shared wrappers for the new read-only gameplay-state view and compatibility shims for older host/event naming.
- `messages/`: Internal gameplay message contracts and dispatch helpers.
- `runtime/`: Core runtime loop, queues, clocks, execution services, and runtime-owned split gameplay state-set storage, including timed-modifier remaining-game-hour projection for smoke/viewer consumers plus the phase-1 site-scene-ready handshake that keeps newly started site runs in `SITE_LOADING` until the host/adapter confirms scene resources are ready, with gameplay-owned state now split from runtime-owned presentation/session caches, routed through `StateManager` ownership/queries, and backed by a dedicated read-only state-view cache/query layer for host inspection.
