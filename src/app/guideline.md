...
- `game_presentation_coordinator.h`: App-layer coordinator declaration for campaign/site bootstrap, projection publishing, panel visibility tracking, and post-message presentation follow-up that used to live on `GameRuntime`.
- `game_presentation_coordinator.cpp`: App-layer implementation of runtime-facing UI/projection/bootstrap publishing, including regional-map and site snapshot emission, site dirty-flush handling, placement-preview projection, HUD/resource cues, panel coordination policy extracted out of `src/runtime/game_runtime.cpp`, and the remaining host-message-side presentation follow-up used to emit immediate UI/storage projections after runtime systems mutate authoritative state.
