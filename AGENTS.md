...
- `refactor.md`: Gameplay state refactor plan for moving authoritative gameplay reads to the exported `Gs1GameStateView` ABI, removing gameplay-owned UI/presentation state and projection-state transfer, and now also tracking the currently incomplete tasks still remaining in that migration, including the completed phone-panel and regional tech-tree session-state ownership cleanup slices plus the completed regional-map scene, regional-selection panel, and site-session shell migration away from dedicated snapshot consumption.
