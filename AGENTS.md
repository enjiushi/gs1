...
- `INLINE_TYPED_GAMEPLAY_MESSAGE_REFACTOR_PLAN.md`: Design and progress-tracking plan for the internal gameplay-DLL message refactor from enum-plus-payload queued/inline `GameMessage` transport toward compile-time typed POD gameplay messages, compile-time ordered system-pack storage, compile-time subscriber derivation, immediate inline internal gameplay dispatch only, host-boundary translation into typed gameplay messages while keeping host/runtime ABI transports stable, and compile-time emitted-runtime-message manifest validation.
