...
- `src/host/`: Engine-agnostic runtime-DLL host bridge code shared by smoke hosts and future engine adapters, now including the first shared engine-message projection cache and drain path used by the Godot adapter plus the shared adapter-metadata catalog loader for presentation-only labels and descriptions.
- `src/runtime/`: Core runtime loop, fixed-step runner, host-event dispatch plus transient phase-control handling, message queues, random service, engine message emission, and the aggregate runtime-owned `GameState` container, now including typed UI-surface visibility messages for engine-owned panel shells alongside the existing semantic panel and progression projections, with `GameRuntime` directly owning both system creation/subscription and the shared gameplay/session state while site systems assemble any narrow file-local access they need from `RuntimeInvocation` instead of requesting a shared runtime-owned site context bundle.
