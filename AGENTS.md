...
- `src/runtime/`: Core runtime loop, fixed-step runner, host-event dispatch plus transient phase-control handling, message queues, random service, and engine message emission, now including typed UI-surface visibility messages for engine-owned panel shells alongside the existing semantic panel and progression projections, with `GameRuntime` directly owning system creation/subscription while site systems assemble any narrow file-local access they need from `RuntimeInvocation` instead of requesting a shared runtime-owned site context bundle.
