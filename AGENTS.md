...
- `engines/godot/`: First-party Godot visual client scaffolding, including `native/` runtime-extension source that now uses a persistent `godot-cpp` runtime node, boots gameplay from the staged `engines/godot/project/gs1/runtime/` package root, loads adapter-owned presentation metadata from the staged `adapter_metadata/` copy, a bootstrap `Gs1GodotDirectorControl` that swaps dedicated main-menu, regional-map, and site-session scenes by gameplay-projected app state, split scene/UI controller roots that separate world/input ownership from panel/runtime-message ownership, and a Godot-object-owning site presenter keyed by gameplay entity/visual IDs with snapshot-safe in-place mutation, plus `project/` Godot scenes, runtime-extension packaging, and engine-specific progression-resource config that maps tech ids plus unlockable content ids to Godot asset paths through the staged `gs1/godot_config/` copy. The regional-map shell now uses an authored dune backdrop, camp-style site markers, heat haze, and a tabbed scrollable tech overlay, while the site shell has a native HUD surface for meters and panel launchers plus projected protection/result modal surfaces. The heavy `engines/godot/project/assets` and `engines/godot/project/addons` payloads are still intended to be hydrated from Cloudflare/GCS rather than stored in Git. The default Godot engine source checkout for adapter work should live at the sibling path `../godot`, with local overrides still allowed through CMake cache variables.
