...
- `src/host/`: Engine-agnostic runtime-DLL host bridge code shared by process/system-test hosts and future engine adapters, including the shared DLL loader, runtime-session wrapper around split phase execution plus typed host-command submission, adapter-config extraction, and adapter-metadata catalog loading for presentation-only labels/descriptions keyed by shared gameplay ids.
