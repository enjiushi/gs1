...
- `GAMEPLAY_DLL_SYSTEM_AUDIT.md`: Source-backed audit of the gameplay DLL runtime-system registry, including each registered system's role, message-by-message internal/host subscription handling, message-by-message emitted gameplay traffic, owned or accessed state, and declared site ECS access masks, now also documenting the runtime-enforced cross-owner mutation guard, const-only non-owner reads, the message-rerouted `CampaignTimeSystem` fix, the lifecycle-helper `CampaignFlowSystem` fix, and the owner-scoped `DeviceMaintenanceSystem` fix.
