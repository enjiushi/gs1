...
- `CMakeLists.txt`: Builds the gameplay DLL plus reusable `gs1_gameplay_core`, `gs1_host_bridge`, and smoke-support libraries; the smoke, visual-smoke, headless game-process, and standalone system-test hosts; runtime regression/perf executables such as the state-manager, message-chain, semantic-message-stack, runtime-system-trait-metadata, projection, timed-modifier, site-message-flow, local-weather, and smoke-host-threading tests; and the pure C++ Godot policy/debug-protocol tests. It also optionally builds the first-party Godot runtime extension under `engines/godot/`, stages `project/content/`, `project/adapter_metadata/`, and Godot-only config into `engines/godot/project/gs1/`, now including the authored Godot prewarm transition catalog under `project/adapter_metadata/`, and wires the shared gameplay/host libraries around `flecs`, `tomlplusplus`, the internal precompiled-header path, and the split runtime state/view infrastructure.
...
- `include/gs1/game_api.h`: C ABI entry points exposed by the gameplay DLL, including runtime creation/destruction, split `run_phase1(...)` / `run_phase2(...)`, runtime-message draining, direct read-only gameplay-state access through `gs1_get_game_state_view(...)`, targeted site-tile lookup through `gs1_query_site_tile_view(...)`, and runtime profiling controls/snapshots.
