...
- `STATE_RESOLVER_ARCHITECTURE_PLAN.md`: Proposed `StateManager` architecture and migration plan for routing cross-owner reads through `StateManager::query(...)`, now also tracking which split-state compatibility and test slices are already complete in the current codebase, distinguishing the finished shared campaign/site hydration-write-back consolidation from the still-unimplemented stronger default-resolver-baseline ideas in the original proposal.
- `COMPILE_TIME_RUNTIME_STATE_PLAN.md`: Architecture plan for moving from the current fixed GS1 `GameState` aggregate to a compile-time game-definition-driven system pack, generated owned-state pack, compile-time validated access traits/profile specializations, and game-specific ABI-safe exported state-view builders that can still feed the Godot adapter.
