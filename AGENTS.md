...
- `third_party/godot-cpp/`: Expected `godot-cpp` checkout location used by the Godot adapter build so the native visual adapter can talk to Godot through normal C++ bindings instead of project scripts or raw engine ABI plumbing; pair it with a sibling Godot source checkout at `../godot` for the default local setup path.
...
- The default local layout is this repo plus a sibling Godot source checkout at `../godot` and a `godot-cpp` checkout at `third_party/godot-cpp`.
