...
- `shared_framework/`: Vendored snapshot of the shared upstream framework repo used across GS-series projects, with `core/runtime/` kept engine-agnostic and reusable Godot-side bootstrap/debug/controller/director helpers living under `engines/godot/`. Read `shared_framework/guideline.md` before drilling deeper.
...
- `src/host/`: GS1-owned host bridge implementation and headers used by the gameplay DLL hosts and Godot adapter after the earlier shared-framework extraction was rolled back for GS1-specific host/session code.
...
- `scripts/`: PowerShell build and test helpers focused on gameplay DLL, process-test, system-test, and Godot adapter workflows, including the dedicated semantic player-journey runner exposed through `run_process_tests.ps1`.
...
- `DIRTY_STATE_SYNC_IMPLEMENTATION_PLAN.md`: Design and progress-tracking plan for replacing frame-wide adapter refreshes with a systematic dirty-state pipeline, including runtime-owned dirty journals, state-set-level invalidation, adapter-side keyed reconciliation, and a small-surface/per-frame-pull exception for trivial UI values.
