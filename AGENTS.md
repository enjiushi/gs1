...
- `GAMEPLAY_DLL_SYSTEM_AUDIT.md`: Source-backed audit of the gameplay DLL runtime-system registry, including each registered system's role, internal/host message subscriptions, emitted gameplay messages, owned or accessed state, and declared site ECS access masks, with the resolved host-reroute cleanup now removed from the audit and the remaining ownership mismatch notes centered on `CampaignTimeSystem`, `CampaignFlowSystem`, and `DeviceMaintenanceSystem`.
