...
- `tests/`: Runtime, headless game-process, smoke, and system test code plus scripted/system-test assets, including the visual smoke viewer-side FPS readout and test-only harnesses that now stand in for the removed production site-system context bundle, the in-place split-state round-trip helpers used by runtime/system test fixtures, and the split inventory-ref call sites used by the last inventory-bridge cleanup. Read `tests/guideline.md` before drilling deeper.
...
- `CMakeLists.txt`: Builds the gameplay DLL plus reusable `gs1_gameplay_core` and `gs1_host_bridge` libraries; the current headless game-process and standalone system-test hosts; the remaining supported runtime regression/perf executables such as the state-manager, semantic-message-stack, projection, and local-weather probes; and the pure C++ Godot policy/debug-protocol tests. It also optionally builds the first-party Godot runtime extension under `engines/godot/`, stages `project/content/`, `project/adapter_metadata/`, and Godot-only config into `engines/godot/project/gs1/`, now including the authored Godot prewarm transition catalog under `project/adapter_metadata/`, and wires the shared gameplay/host libraries around `flecs`, `tomlplusplus`, the internal precompiled-header path, and the split runtime state/view infrastructure while the old runtime-message transport-dependent smoke/runtime test targets are being removed.
...
- `include/gs1/game_api.h`: C ABI entry points exposed by the gameplay DLL, including runtime creation/destruction, split `run_phase1(...)` / `run_phase2(...)`, direct read-only gameplay-state access through `gs1_get_game_state_view(...)`, targeted site-tile lookup through `gs1_query_site_tile_view(...)`, and runtime profiling controls/snapshots after the public runtime-message pop entry point was removed.
...
- `include/gs1/system_test_api.h`: Public standalone system-test discovery/run ABI used by the linked system-test host and exported test-asset entry points.
- `include/gs1/types.h`: Public API data types, typed host-command/request structs, runtime-facing enums, split host/runtime phase request/result structs, runtime profiling structs, claimed-task/phone-panel transport fields, site-weather event timeline transport fields, typed UI-surface visibility transport, tech-tree view/selection action identifiers, and harvest-action plus harvested-output transport payloads.
...
- `src/events/`: Legacy placeholder folder retained only for guidance; gameplay coordination uses `src/messages/` `GameMessage` flow rather than a separate runtime-event layer.
- `src/host/`: Engine-agnostic runtime-DLL host bridge code shared by smoke hosts and future engine adapters, including the shared DLL loader, runtime-session wrapper around split phase execution plus state-view/tile-query calls, adapter-config extraction, and adapter-metadata catalog loading for presentation-only labels/descriptions keyed by shared gameplay ids.
- `src/runtime/`: Core runtime loop, fixed-step runner, typed host-command dispatch, internal gameplay-message routing, random/profiling services, split runtime/campaign/site state-set storage, `StateManager` ownership/query wiring, and the dedicated exported gameplay-state view plus tile-query helper layer used by hosts and adapters, with the current direction moving internal `GameMessage` handling toward inline nested dispatch while public host commands and runtime-to-host message transports remain separate.
- `src/site/`: Active site-run state and site systems for dedicated site time progression, actions, weather, owner-specific plant/device local-weather contribution passes, ecology, directional local wind shelter, inventory, economy, task board, camp durability, harvested-output flow, three-depth excavation-action resolution plus occupier-hidden excavation visuals, and completion/failure flow, with gameplay-owned site projection dirty caches, the last transitional dirty-mark helper shims, and the dead `SiteRunState` inventory bridge helpers now removed from the active site runtime path.
- `src/support/`: Shared support types such as typed IDs.
- `src/testing/`: Runtime-side system-test asset loading and test-registration helpers shared by the exported DLL hooks and the standalone linked system-test host.
...
- `engines/godot/`: First-party Godot visual client scaffolding, including a native `godot-cpp` runtime extension that boots gameplay from the staged package root, wraps the shared runtime session, drains semantic runtime messages, rebuilds scene/panel state from `Gs1GameStateView` plus targeted tile queries on coarse dirty signals, owns stable gameplay-id-to-Godot-object reconciliation for regional/site visuals, and can optionally expose a loopback-only semantic debug HTTP control port. The folder also contains the Godot project shell, extension packaging, staged runtime/config roots, authored main-menu/regional/site scenes and sub-scenes, and Godot-only progression/render-resource TOML config.
- `tests/smoke/`: Smoke host, runtime DLL loader/session bridge, script runner, live-state JSON, live HTTP server, and visual smoke UI, including the viewer-side FPS counter, right-click harvest and excavate actions, headless silent-onboarding automation, browser tech-tree/modifier metadata merged from gameplay content plus adapter-owned metadata, and the smoke-host `SITE_SCENE_READY` auto-ack path used when no real async scene load exists.
...
- `GAMEPLAY_DLL_SYSTEM_AUDIT.md`: Source-backed audit of the gameplay DLL runtime-system registry, including each registered system's role, message-by-message internal/host subscription handling, message-by-message emitted gameplay traffic, owned or accessed state, and declared site ECS access masks, including runtime-originated boot-message exceptions such as `OpenMainMenu`, producer-gap notes where campaign-flow `GameMessage` subscriptions are bypassed by the live host-action path, the current removal of site-success campaign/faction reputation awards from `CampaignFlowSystem`, and the remaining design tension around site-attempt/result flow versus the intended simpler enter/leave site-session model with completed sites still non-reenterable, while also documenting the runtime-enforced cross-owner mutation guard, const-only non-owner reads, the message-rerouted `CampaignTimeSystem` fix, the lifecycle-helper `CampaignFlowSystem` fix, and the owner-scoped `DeviceMaintenanceSystem` fix.
- `HOST_COMMAND_ABI_REFACTOR_PLAN.md`: Design and progress-tracking plan for replacing the public host-to-runtime `Gs1HostMessage` enum-plus-union transport with concrete typed host command entry points, keeping exported ABI functions concrete while allowing internal template/overload translation helpers, preserving the separation between public host commands and internal typed gameplay messages, and migrating host bridge/runtime code toward direct typed command submission plus compile-time internal gameplay subscriber dispatch.
- `INLINE_TYPED_GAMEPLAY_MESSAGE_REFACTOR_PLAN.md`: Design and progress-tracking plan for the internal gameplay-DLL message refactor from enum-plus-payload queued/inline `GameMessage` transport toward compile-time typed POD gameplay messages, compile-time ordered system-pack storage, compile-time subscriber derivation, immediate inline internal gameplay dispatch only, host-boundary translation into typed gameplay messages while keeping host/runtime ABI transports stable, and compile-time emitted-runtime-message manifest validation, now also tracking the landed runtime-boundary translation of the campaign-flow, technology, task-board, site-modifier, and economy-phone gameplay-action host families plus the follow-up cleanup that routes those migrated families through direct typed handlers inside the owning systems.
...
- `PROGRESSION_REFACTOR_PLAN.md`: Living design and checklist for the medium-scope generic progression refactor, including target generic ids/messages/tables, migration slices, assertion rules, and the explicit list of deeper-cleanup work intentionally left undone after the medium pass.
...
- `RUNTIME_MESSAGE_REMOVAL_REMAINING_WORK.md`: Checkpoint note for the in-progress full runtime-message transport removal, listing what has already been removed, what dead declarations and legacy smoke/runtime files still remain, and which supported targets were verified at this stage.
...
- Internal gameplay-message dispatch must route a message only to systems that explicitly subscribe to that message type.
