...
- `ECS_REFACTOR_PLAN.md`: Draft working plan for consolidating active site gameplay data around ECS-first storage, including grouped tile/device/worker components, tile-grid random access, occupier modeling, the first-pass one-entity-one-storage-grid inventory direction, craft migration, phased rollout slices, and the still-open questions being refined before implementation begins.
- `TODO_REFACTOR.md`: Working checklist for the split-state/runtime refactor, including what is already complete in the production no-compat path; the runtime game-state view cache now rebuilds directly from flat state sets, while the remaining narrow non-production cleanup targets sit in test fixtures/tests plus the last `SiteRunState` inventory bridge helpers.
