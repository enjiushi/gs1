...
- `include/gs1/state_view.h`: Public ABI-safe gameplay state-view declarations used by hosts to read authoritative campaign/site state through trivial pointer/count/query-facing structs without exposing internal runtime containers.
...
- `refactor.md`: Gameplay state refactor plan for moving authoritative gameplay reads to the exported `Gs1GameStateView` ABI, removing gameplay-owned UI/presentation state and projection-state transfer, and now also tracking the currently incomplete tasks still remaining in that migration, including the completed phone-panel and regional tech-tree session-state ownership cleanup slices plus the completed regional-map scene, regional-selection panel, site-session shell, and site-scene presenter migration away from dedicated snapshot consumption.
