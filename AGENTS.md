...
- `include/gs1/types.h`: Public API data types, message/event payload structs, runtime-facing enums, split host/runtime phase request/result structs, runtime profiling structs, claimed-task/phone-panel transport fields, site-weather event timeline transport fields, typed UI-surface visibility transport, tech-tree view/selection action identifiers, and harvest-action plus harvested-output transport payloads.
...
- `src/runtime/`: Core runtime loop, fixed-step runner, host-message dispatch, internal gameplay-message routing, random/profiling services, split runtime/campaign/site state-set storage, `StateManager` ownership/query wiring, and the dedicated exported gameplay-state view plus tile-query helper layer used by hosts and adapters, with the current direction moving internal `GameMessage` handling toward inline nested dispatch while host and runtime message transports remain separate.
