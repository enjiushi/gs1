...
- `types.h`: Shared API structs, payloads, engine-message transport layouts, handles, and enums used by runtime callers, including authoritative phone home/app-panel section transport plus tracked open-state flags, tracked inventory-view events for both the worker pack and device storage panels, claimed-task list-kind projection, site-tile local-wind plus ecology-meter projection for viewer-reactive plant motion and terrain shading, campaign cash/total-reputation projection, weather-event timeline payloads, regional-map tech-tree UI setup/action identifiers including tech refunds, and harvest-action plus harvested-output transport payloads.
