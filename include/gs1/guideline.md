...
- `game_api.h`: Main exported gameplay entry points and host-facing function declarations, including concrete typed host-command submit functions, direct read-only gameplay-state access through `gs1_get_game_state_view(...)`, and targeted tile lookup through `gs1_query_site_tile_view(...)`.
...
- `types.h`: Shared API structs, payloads, engine-message transport layouts, handles, and enums used by runtime callers, including the host-supplied shared project-config root pointer in `Gs1RuntimeCreateDesc` plus optional adapter-config JSON, typed host-command/request structs for UI actions, movement, site actions, host-local storage views, and inventory taps, compact progression-entry transport for regional-map research/unlock data, tracked inventory-view events for worker-pack or device-storage open/close intents plus a neutral host inventory-slot-tap command that leaves use-versus-transfer resolution inside gameplay, and the reduced semantic runtime-message families used by the active Godot path now that host-owned phone/protection/tech-tree panel session state no longer rides the gameplay ABI.
