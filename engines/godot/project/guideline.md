...
- `scenes/`: Godot-authored scenes for the client shell, now split between a bootstrap router scene, dedicated main-menu/regional-map/site-session shells, the reusable site HUD, the regional-map HUD panel scene, and additional authored regional-map support scenes for the dune-basin backdrop and camp-style site marker, all following gameplay-projected app-state/UI setup messages while sharing the project-authored desert shell theme resource, with the menu now presenting its authored 3D vista through a dedicated `SubViewport` layer plus the shared heat-haze language.
