...
- `content_resources.toml`: Godot-only general content resource map keyed by shared `Item`, `Plant`, `Structure`, and `Recipe` ids so inventory, craft, placement-preview, and other non-progression UI surfaces can bind engine-local icons without moving names or descriptions out of the shared gameplay content tables.
